...
function resolveCachedConnectionForModel(connections: LlmConnectionSettings[], model: string): LlmConnectionSettings | null {
...
export async function resolveConnectionForModel(connections: LlmConnectionSettings[], model: string): Promise<LlmConnectionSettings | null> {
	const cached = resolveCachedConnectionForModel(connections, model)
	if (cached) {
		return cached
	}

	const promises = connections.map((connection) => getAvailableOpenaiModels(connection)
		.then(models => {
			if (!models) {
				return
			}
